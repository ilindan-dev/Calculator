#include "calculator/plugin.hpp"
#include <cmath>

#ifndef M_PI
#define M_PI 3.14159265358979323846
#endif

double tg_func(const double* args, int argCount) {
    if (argCount != 1) {
        return NAN;
    }

    const double degrees = args[0];
    const double radians = degrees * (M_PI / 180.0);

    const double cos_val = std::cos(radians);
    if ( std::abs(cos_val) < 1e-15) {
        return NAN;
    }

    return std::sin(radians)/cos_val;
}

extern "C" PLUGIN_API void registerPlugin(ICalculatorRegistrar* registrar) {
    OperationInfo info = {};
    info.canonicalName = "tg";
    info.function = tg_func;
    info.numArguments = 1;
    info.type = OperationType::Function;
    info.symbol = "tg";

    registrar->registerOperation(info);
}